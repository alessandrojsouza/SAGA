...
void loop() {
  ...
  if (!client) {
    ...
    if ( !getPage((float)vazao, (int)estadoEletroBomba, (int)estadoEletroDreno, (int)bomba, (int)sensCisterna, (int)sensPoco)) {
      Serial.println("Falha na requisição GET");
    }
    if ( sensCisterna == 100 || sensCisterna == 0) {
      if ( !getPage2((int)sensCisterna) ) {
        Serial.println("Falha na requisição GET");
      }
    }
    if ( sensPoco == 100 || sensPoco == 0) {
      if ( !getPage3((int)sensPoco) ) {
        Serial.println("Falha na requisição GET");
      }
    }
    return;
...
bool getPage(float vazao, int estadoEletroBomba, int estadoEletroDreno, int bomba, int sensCisterna, int sensPoco) {
  if ( !client.connect(ip, http_port) ) {
    Serial.println("Falha na conexao com o site ");
    return false;
  }
  String param = "?vazao=" + String(vazao) + "&" + "eletroBomba=" + String(estadoEletroBomba) + "&" + "eletroDreno=" + String(estadoEletroDreno) + "&" + "bomba=" + String(bomba) + "&" + "sensCisterna=" + String(sensCisterna) + "&" + "sensPoco=" + String(sensPoco); //Parâmetros com as leituras
...
bool getPage2(int sensCisterna) {
  if ( !client.connect(ip, http_port) ) {
    Serial.println("Falha na conexao com o site ");
    return false;
  }
  ...
  Serial.println(param);
  client.println("GET /registra-historico-niveis.php" + param);
  client.println("Host: ");
  client.println(http_site);
  client.println("Connection: close");
  client.println();
...
bool getPage4(int bomba) {
  ...
  Serial.println(param);
  client.println("GET /registra-historico-bomba.php" + param);
  client.println("Host: ");
  client.println(http_site);
  client.println("Connection: close");
  client.println();
